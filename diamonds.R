...
colnames(diamonds) <- c("Carat", "Color", "Clarity", "Cert", "Price")
head(diamonds)

#EDA
#Scatterplot of Carat and Price
qplot(diamonds$Carat, diamonds$Price, geom="point", ylab="Diamond Price", xlab="Carats", main="Data Before Transformation")

#Scatterplot of ln(Carat) and ln(Price)
qplot(log(diamonds$Carat), log(diamonds$Price), geom="point", xlab="Diamond Size (in log(carats))", 
      ylab="Price (in log(Singapore $))", main="Data After Transformation")

#Analysis

#The response variable is the ln(Price of the Diamond).
#The explanatory variable is the ln(Diamond Size, in Carats).

#Model:  ln(Price) = beta0 + beta1*ln(Carat) + epsilon, where epsilon ~ N(0, sigma2)
#take the exp of both sides to get the model in terms untransformed Price.
#Model: Price = exp(beta0)*(Carat^beta1) * epsilon2   where epsilon2 ~ lognormal

#Fit the Model
diamonds.out <- lm(log(Price)~log(Carat), data=diamonds)

#Table of parameter estimates and standard errors
summary(diamonds.out)


#The value beta1.hat is an estimate of the slope of a logtransformed model. 
#For a 1% increase in Carat, we estimate an expected increase in Price of 1.54%
#(Because the model is truly multiplicative, we should describe the slope in multiplicative terms)

#95% Confidence Interval for beta1
confint(diamonds.out)
#Conclusion: We are 95% confident that the true beta1 is with the range (1.5008, 1.5737). This means 
#that there is virtually no chance that there is not an association between lnCarat and lnPrice, because
# 0 is not in that range. 

#The estimated model for lnPrice.hat
# lnPrice.hat = 9.13 + 1.54* lnCarat

#Publication Graphic for jewelry store manager showing line and uncertainty associated with line
qplot(log(Carat), log(Price), data=diamonds, geom="smooth", formula=y~x, method="lm", se=TRUE,
      xlab="Diamond Size (Carats)", 
      ylab="Diamond Price (Singapore $)")

#Predict a 95% prediciton interval for a couple buying a one-carat diamond
#log Price
...
# r^2 from summary <- based on logtransform
#to calculate the actual, r^2 = 1- sum((yi-yi.hat)^2)/sum((yi-yi.actual)^2)
#Real R Squared
1-sum((diamonds$Price - exp(predict(diamonds.out)))^2) / sum((diamonds$Price-mean(diamonds$Price))^2)
# 90.53% of the variation in diamond price can be explained by Diamond Size (Carats).

#Summary Statistics of the Absolute Prediciton Error
summary( abs( diamonds$Price - exp(predict(diamonds.out) )) )

#graphic of uncertaintly with predicitions
